...
- **Project detection** — `resolveProjectRoot()` (`src/services/projectDetection.ts`) honours `erdStudio.projectPath`, then workspace folder roots, then a depth-3, name-ordered search for a dbt project (`dbt_project.yml`) or a native SQLMesh project (`config.yaml`/`config.yml` with `gateways`/`model_defaults`/`default_gateway`, a `config.py` importing `sqlmesh`, or a saved `{semanticDir}/sqlmesh.json`), skipping hidden directories, `node_modules`, `dbt_packages`, `target`, `dist`, `venv`. `erdStudio.provider` pins one framework; `auto` prefers dbt. **Activation events are deliberately narrow** — `**/dbt_project.yml`, `**/.erd-studio/layers.json`, `**/.erd-studio/sqlmesh.json` — never a generic `**/config.yaml` / `**/config.py`, which would activate (and toast "no project found") in nearly every workspace; a brand-new SQLMesh project activates through the sidebar view or any ERD Studio command instead. With no project found, `registerFallbackCommands()` registers every contributed command (plus aliases) with a "no dbt project" warning and a stub custom editor, so nothing reports "command not found".
